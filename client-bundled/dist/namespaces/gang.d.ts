import { IPlayer } from '@risinglife/redm-shared';
/**
 * No comment provided
 *
 * Hash: 0x901E0DC25080C8B9 | Since: 1207 | API-Set: unknown
 */
export declare function networkGetId(player: number | string | IPlayer): any;
/**
 * No comment provided
 *
 * Hash: 0x4BE6C13A45CCA8EC | Since: 1207 | API-Set: unknown
 */
export declare function networkGetLeader(gangId: any): number | string;
/**
 * No comment provided
 *
 * Hash: 0x149A2751AB66AC02 | Since: 1207 | API-Set: unknown
 */
export declare function networkGetNumMembers(gangId: any): number;
/**
 * No comment provided
 *
 * Hash: 0x0F99F6436528A089 | Since: 1207 | API-Set: unknown
 */
export declare function networkIsActive(gangId: any): boolean;
/**
 * No comment provided
 *
 * Hash: 0xD6F6ACF4392187FB | Since: 1207 | API-Set: unknown
 */
export declare function networkIsIdValid(gangId: any): boolean;
/**
 * No comment provided
 *
 * Hash: 0x93A91A351A07360E | Since: 1207 | API-Set: unknown
 */
export declare function networkIsInSession(gangId: any): boolean;
/**
 * No comment provided
 *
 * Hash: 0x424B17A7DC5C90BC | Since: 1207 | API-Set: unknown
 */
export declare function networkIsLeader(player: number | string | IPlayer): boolean;
/**
 * No comment provided
 *
 * Hash: 0x3F59FE6F37869576 | Since: 1207 | API-Set: unknown
 */
export declare function networkIsInSame(player1: number | string | IPlayer, player2: number | string | IPlayer): boolean;
/**
 * No comment provided
 *
 * Hash: 0x0E5C9FB9ED5DFF1C | Since: 1207 | API-Set: unknown
 */
export declare function _0x0E5C9FB9ED5DFF1C(): any;
/**
 * No comment provided
 *
 * Hash: 0x1F11702DDBD915C6 | Since: 1207 | API-Set: unknown
 */
export declare function _0x1F11702DDBD915C6(): any;
/**
 * No comment provided
 *
 * Hash: 0x2F7EB8B6F6AFE79C | Since: 1207 | API-Set: unknown
 */
export declare function _0x2F7EB8B6F6AFE79C(): any;
/**
 * No comment provided
 *
 * Hash: 0x3ADC71A66356D706 | Since: 1207 | API-Set: unknown
 */
export declare function _0x3ADC71A66356D706(): any;
/**
 * No comment provided
 *
 * Hash: 0x48D82C83987E18E4 | Since: 1207 | API-Set: unknown
 */
export declare function _0x48D82C83987E18E4(): any;
/**
 * No comment provided
 *
 * Hash: 0x51C5EF47086AA0D7 | Since: 1207 | API-Set: unknown
 */
export declare function _0x51C5EF47086AA0D7(): any;
/**
 * No comment provided
 *
 * Hash: 0x53A94294FDDCF98C | Since: 1207 | API-Set: unknown
 */
export declare function _0x53A94294FDDCF98C(): any;
/**
 * No comment provided
 *
 * Hash: 0x6102830F764B3DE1 | Since: 1207 | API-Set: unknown
 */
export declare function _0x6102830F764B3DE1(player: number | string | IPlayer): boolean;
/**
 * No comment provided
 *
 * Hash: 0x644E02F24F9D4E98 | Since: 1207 | API-Set: unknown
 */
export declare function _0x644E02F24F9D4E98(): any;
/**
 * No comment provided
 *
 * Hash: 0x7933754F260B428A | Since: 1207 | API-Set: unknown
 */
export declare function _0x7933754F260B428A(player: number | string | IPlayer): any;
/**
 * No comment provided
 *
 * Hash: 0x7BAA30C9BBE8AEE7 | Since: 1207 | API-Set: unknown
 */
export declare function _0x7BAA30C9BBE8AEE7(): any;
/**
 * No comment provided
 *
 * Hash: 0xA9CEAE8D6637FBAD | Since: 1207 | API-Set: unknown
 */
export declare function _0xA9CEAE8D6637FBAD(): void;
/**
 * No comment provided
 *
 * Hash: 0xAD22AB64FA428DF3 | Since: 1207 | API-Set: unknown
 */
export declare function _0xAD22AB64FA428DF3(): void;
/**
 * No comment provided
 *
 * Hash: 0xAFD3599A3CC5637D | Since: 1207 | API-Set: unknown
 */
export declare function _0xAFD3599A3CC5637D(): any;
/**
 * No comment provided
 *
 * Hash: 0xB22B1D9F74095382 | Since: 1207 | API-Set: unknown
 */
export declare function _0xB22B1D9F74095382(): void;
/**
 * No comment provided
 *
 * Hash: 0xB38C256498748413 | Since: 1207 | API-Set: unknown
 */
export declare function _0xB38C256498748413(): void;
/**
 * No comment provided
 *
 * Hash: 0xC81A9E2C8EFD28D5 | Since: 1207 | API-Set: unknown
 */
export declare function _0xC81A9E2C8EFD28D5(): void;
/**
 * _NETWORK_GET_* or _NETWORK_IS_*
 *
 * Hash: 0xDA801F7F6A5278D3 | Since: 1207 | API-Set: unknown
 */
export declare function _0xDA801F7F6A5278D3(player: number | string | IPlayer): boolean;
/**
 * No comment provided
 *
 * Hash: 0xE4C64CD37CB176AA | Since: 1207 | API-Set: unknown
 */
export declare function _0xE4C64CD37CB176AA(): any;
/**
 * No comment provided
 *
 * Hash: 0xEE4F20004D0288B7 | Since: 1207 | API-Set: unknown
 */
export declare function _0xEE4F20004D0288B7(): void;
/**
 * No comment provided
 *
 * Hash: 0xFA7C5B7E087A4CEB | Since: 1207 | API-Set: unknown
 */
export declare function _0xFA7C5B7E087A4CEB(): any;
/**
 * No comment provided
 *
 * Hash: 0xCE88A261DCBBA0D9 | Since: 1207 | API-Set: unknown
 */
export declare function networkGetLeaderHandle(gangId: any): [boolean, any];
/**
 * No comment provided
 *
 * Hash: 0xD1BF325C8252A982 | Since: 1207 | API-Set: unknown
 */
export declare function networkGetMembers(gangId: any): [number, any];
/**
 * No comment provided
 *
 * Hash: 0x9970AE8C3D706139 | Since: 1207 | API-Set: unknown
 */
export declare function networkGetPrivacy(): number;
/**
 * No comment provided
 *
 * Hash: 0x853B0FA4D8732C57 | Since: 1207 | API-Set: unknown
 */
export declare function networkGetSize(gangId: any): number;
/**
 * No comment provided
 *
 * Hash: 0x9BE7DCB22D32CCBE | Since: 1207 | API-Set: unknown
 */
export declare function networkIsMember(gangId: any, player: number | string | IPlayer): boolean;
/**
 * No comment provided
 *
 * Hash: 0xFCF96CCBD81B24C8 | Since: 1207 | API-Set: unknown
 */
export declare function networkIsOpen(gangId: any): boolean;
/**
 * No comment provided
 *
 * Hash: 0x81FB74C83C2ED69F | Since: 1207 | API-Set: unknown
 */
export declare function networkIsInMy(player: number | string | IPlayer): boolean;
/**
 * banTimeSeconds is 120 in R* Scripts
 *
 * Hash: 0xCD9E2D9BC52FD80F | Since: 1207 | API-Set: unknown
 */
export declare function networkKickMember(player: number | string | IPlayer, banTimeSeconds: number): void;
/**
 * No comment provided
 *
 * Hash: 0x0A04A07BC3074EDB | Since: 1207 | API-Set: unknown
 */
export declare function networkLeave(disband: boolean): void;
/**
 * Returns true if join succeeded, false if failed.
 *
 * Hash: 0xC0474C8BCF6787AD | Since: 1207 | API-Set: unknown
 */
export declare function networkRequestJoin(gangId: any): boolean;
/**
 * No comment provided
 *
 * Hash: 0xC5BF29F4035277C2 | Since: 1207 | API-Set: unknown
 */
export declare function networkSetPrivacy(privacyType: number): boolean;
/**
 * No comment provided
 *
 * Hash: 0x833D8268D51B4522 | Since: 1207 | API-Set: unknown
 */
export declare function networkSetSize(size: number): boolean;
/**
 * openStatus = true -> sets privacyType = 2 (PUBLIC_ADVERTISED)
 * openStatus = false -> sets privacyType = 1 (INVITE_ONLY)
 *
 * campSize: NET_CAMP_SIZE_SMALLEST = 4, NET_CAMP_SIZE_LARGEST = 7
 *
 * Hash: 0xD1A226F2E05E58FC | Since: 1207 | API-Set: unknown
 */
export declare function networkStart(openStatus: boolean, campSize: number): void;
