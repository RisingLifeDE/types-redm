/**
 * No comment provided
 *
 * Hash: 0x07AD9E43FD478527 | Since: 1207 | API-Set: unknown
 */
export declare function _0x07AD9E43FD478527(): boolean;
/**
 * No comment provided
 *
 * Hash: 0xA46FD001D1BE896C | Since: 1207 | API-Set: unknown
 */
export declare function _0xA46FD001D1BE896C(): string;
/**
 * No comment provided
 *
 * Hash: 0x466BC8769CF26A7A | Since: 1207 | API-Set: unknown
 */
export declare function decrementCashBalance(amount: number): boolean;
/**
 * No comment provided
 *
 * Hash: 0x0C02DABFA3B98176 | Since: 1207 | API-Set: unknown
 */
export declare function getCashBalance(): number;
/**
 * No comment provided
 *
 * Hash: 0xBC3422DC91667621 | Since: 1207 | API-Set: unknown
 */
export declare function incrementCashBalance(amount: number, addReason: number | string): boolean;
/**
 * No comment provided
 *
 * Hash: 0x8A67120DBC299525 | Since: 1207 | API-Set: unknown
 */
export declare function networkGetCashBalance(): number;
/**
 * No comment provided
 *
 * Hash: 0x282D36FF103D78DF | Since: 1207 | API-Set: unknown
 */
export declare function networkGetStringCashBalance(): string;
/**
 * No comment provided
 *
 * Hash: 0xAEC5F0119867E457 | Since: 1207 | API-Set: unknown
 */
export declare function networkIsBalanceNotLessThan(cashBalance: number, goldBarBalance: number): boolean;
